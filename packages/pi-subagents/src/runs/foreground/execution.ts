	...
	const exitCode = await new Promise<number>((resolve) => {
		...
		const processLine = (line: string) => {
			...
			if (evt.type === "tool_result_end" && evt.message) {
				...
				// Fork: the shared, id-deduplicated tool-result tail (message push,
				// budget, mutating-failure tracking, output) lives in handleToolResult;
				// hosts may deliver the same result via message_end(role=toolResult).
				handleToolResult(evt.message, now);
			}
		};
