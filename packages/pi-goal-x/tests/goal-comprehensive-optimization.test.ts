...
test("task and prompt caches observe nested edits, reparenting, focus and settings without retaining mutable inputs", () => {
 const f = fixture(); try {
  ...
  const old = taskIndex(tasks); const original = goalContextMessagePrompt(goal);
  ...
  assert.match(goalContextMessagePrompt(goal), /Revised contract/); assert.notEqual(goalContextMessagePrompt(goal),original);
  ...
  goal.currentTaskId = "n"; assert.match(goalContextMessagePrompt(goal), /Current: n/);
  assert.doesNotMatch(goalContextMessagePrompt(goal,{disableTasks:true}), /TASK LIST/);
...
test("goal detail pages bound oversized task text and point to lossless retrieval", () => {
 const f=fixture(); try {
