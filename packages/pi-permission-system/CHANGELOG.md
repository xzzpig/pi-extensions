...
- Synced the fork with upstream pi-permission-system v31.0.2 while preserving the configurable wrapper-floor (fallback/always) divergence. Adopted upstream: directional path surfaces with a read/write capability axis and wrapper transparency (#806/#807/#803), glob-bearing path token projection (#821), flag/arity parse fixes (#823), command gating in control-flow bodies, function definitions, declarations and assignments (#742), decision attribution with `authorizer_allowed` / `authorizer_denied` resolutions (#772), deny-reason forwarding (#844), removal of the process-root service accessors (#796), the unguarded-subagent-child alarm (#792), extractor/formatter exposure with fact-shaping inheritance (#793), named permission surfaces (#808), per-pattern session approval surfaces and direction-proven external path grants (#810), `isToolFullyDenied` cross-extension pre-filtering (#815), both-directions session grants at the ask prompt (#813), statement-operand gating for case subjects and for/select operands (#839), unresolved-redirect handling (#814), and the fail-closed prompt for unresolvable bash parses (#840). Fork additions kept: `permissions:forwarded_decision` broadcast, injectable response writer, payload-less wrapper adjudication, and terminal permission notifications.
...
- Synced the fork with upstream pi-permission-system v25.3.0 while preserving the configurable wrapper-floor (fallback/always) divergence; adopted upstream #710 bounded dialog, #744 structured payload, #712 deny-first, #713 executedUnit display, #741 nested-execution module.
...
- Synced the fork with upstream pi-permission-system v25.2.0 while preserving the local wrapper-floor behavior.
