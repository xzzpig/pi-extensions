...
describe("shell-tool alias gating (#574)", () => {
	...
	it("floors an indirection wrapper (sudo) in an aliased command in 'always' mode (#490)", async () => {
		const { handler, events } = makeHandler({
			shellTools: execShellTools,
			tools: ["exec_command"],
	...
	it("does not floor a resolved sudo wrapper in fallback mode", async () => {
		const { handler, events } = makeHandler({
			shellTools: execShellTools,
			tools: ["exec_command"],
			prompter: denyingPrompter(),
	...
	it("floors an opaque-payload wrapper (bash -c) in an aliased command in 'always' mode (#481)", async () => {
		const { handler, events } = makeHandler({
			shellTools: execShellTools,
			tools: ["exec_command"],
			prompter: denyingPrompter(),
