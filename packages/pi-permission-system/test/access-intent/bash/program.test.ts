...
describe("BashProgram", () => {
  ...
  describe("commands", () => {
    ...
    describe("opaque-payload wrappers", () => {
      it.each([
        ['bash -c "rm -rf /"', 'bash -c "rm -rf /"', "rm -rf /"],
        ['sh -c "rm -rf /"', 'sh -c "rm -rf /"', "rm -rf /"],
        ['dash -c "rm -rf /"', 'dash -c "rm -rf /"', "rm -rf /"],
        ['zsh -c "rm -rf /"', 'zsh -c "rm -rf /"', "rm -rf /"],
        ['ksh -c "rm -rf /"', 'ksh -c "rm -rf /"', "rm -rf /"],
        ['eval "rm -rf /"', 'eval "rm -rf /"', "rm -rf /"],
        ['/bin/bash -c "rm -rf /"', '/bin/bash -c "rm -rf /"', "rm -rf /"],
        ['bash -ec "rm -rf /"', 'bash -ec "rm -rf /"', "rm -rf /"],
      ...
      it("splits a chained opaque payload into its own units", async () => {
        const program = await BashProgram.parse(
          'eval "cd /tmp && rm -rf /"',
          normalizer,
        );
    ...
    describe("indirection wrappers", () => {
      ...
      // readers, so the wrapper unit also carries the floor exemption (#803)
      // while the inner command is still emitted as its own unit (fork).
      it.each([
        ...
        async (command, executedUnit) => {
          ...
          expect(program.commands()).toEqual([
            ...
            { text: executedUnit, context: "wrapper_indirection" },
          ]);
        },
      );

      it("skips an indirection wrapper's option values and duration when locating the inner command", async () => {
        const program = await BashProgram.parse(
          "timeout -k 5 570 nix eval .#x --json",
          normalizer,
        );
        expect(program.commands()).toEqual([
          {
    ...
    describe("floor exemption", () => {
      /**
       * The exemption recorded for each unit of a parsed command, skipping
       * the inner units the fork additionally emits from inside wrappers
       * (`wrapper_payload` / `wrapper_indirection`) — the exemption is a
       * fact about wrapper units, and the inner units are covered by the
       * emission tables above.
       */
      async function exemptions(
        ...
        return program
          ...
          .filter(
            (unit) =>
              unit.context !== "wrapper_payload" &&
              unit.context !== "wrapper_indirection",
          )
          .map((unit) => unit.floorExemption);
      }
