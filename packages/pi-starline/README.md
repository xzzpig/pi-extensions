...
(`tuiMode: "fullscreen"`) through the [`@xzzpig/pi-mouse-events`](https://www.npmjs.com/package/@xzzpig/pi-mouse-events)
extension, which owns the renderer prototype and dispatches mouse events to
components — the extension API stops at high-level hooks, and the selection
hint, click-to-expand and clean-copy all need to reach into the renderer
itself. Install it alongside Starline; without it Starline's mouse features
are simply off (everything else keeps working).
